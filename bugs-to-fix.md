...
### 1. The write mutex permanently breaks after the first error (`src/lib/server-db.ts`)
```js
...
### 2. Committee-assigned tasks are invisible to everyone (`canViewTask` in `local-data.ts`)
```js
...
### 3. Cross-device polling never actually updates an open screen
...
### 4. Test-persona tiers contradict the app's own role/tier table
...
### 5. Public form slugs aren't checked for uniqueness server-side
...
### 6. Settings → Change Password is fully decorative
...
### 7. Dead, un-mutex'd write path still exists (`src/app/api/data/route.ts`)
