...
// (src/lib/event-social-scheduler.ts), and the daily 1-day-before-deadline
// task reminder scheduler (src/lib/task-deadline-reminder-scheduler.ts)
// rather than requiring an external VPS crontab entry, since this app runs
...
export async function register() {
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    ...
    const { startTaskDeadlineReminderScheduler } = await import('./lib/task-deadline-reminder-scheduler');
    startTaskDeadlineReminderScheduler();
  }
}
