...
export async function POST(
  ...
  try {
    ...
    await createActivationTokenAndSendEmail({ id: member.id, name: member.name, email: member.email });
    return NextResponse.json({ success: true, message: `Welcome email resent to ${member.email}.` });
  } catch (err: any) {
